...
import time
from pick_n_place import move_prismatic, pinza, move_arm, twist_pinza
...
def run_sequence(robotId, cubeId, index):
    move_prismatic(robotId, ARM_TO_CUBE, speed=1)
    pinza(robotId, CLOSE_POSE, speed=0.7)
    move_prismatic(robotId, ARM_HOME, speed=1, ignore_body=cubeId)
    ...
    move_prismatic(robotId, CUBE_TO_CHASSIS, speed=1, ignore_body=cubeId)
